...
describe('Mobile bottom sheet swipe-up gesture', () => {
    ...
    it('renders the swipe hit zone only on touch devices at the mobile breakpoint', () => {
        ...
        // the home-indicator clearance into its own height, so the swipe
        // zone has to clear both the tab-bar height AND that clearance to
        // land at the tab bar's top edge. The clearance is the shared
        // --mobile-bottom-inset token rather than raw
        // env(safe-area-inset-bottom), which reads 34px in iOS standalone
        // against Safari's 0 (see tests/mobileTabBarStandaloneInset.test.js).
        const block = css.match(/\.sheetSwipeZone\s*\{[^}]*\}/);
        expect(block).toBeTruthy();
        expect(block[0]).toMatch(/position:\s*absolute/);
        expect(block[0]).toMatch(/bottom:\s*calc\(\s*var\(--mobile-tab-h[^)]*\)\s*\+\s*var\(--mobile-bottom-inset[^)]*\)\s*\)/);
