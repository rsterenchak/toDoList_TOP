...
- [x] **[MEDIUM]** Fix claudeTabGroup toggle to respond to light theme in CSS
  - Type: bug
...
- [x] **[LOW]** Fix `.todoMdViewerRunBtn` to render as a ghost button in light mode
  - Type: bug
...
- [x] **[LOW]** Add purple border and icon tint on hover to Claude sheet composer buttons
  - Type: feature
...
- [x] **[LOW]** Add purple outer glow to `claudeComposerMic` button to match other composer buttons
...
- [x] **[MEDIUM]** Match `.claudeComposerAttach:hover` styling on `.claudeComposerMic:hover`
  - Type: bug
...
- [x] **[MEDIUM]** Auto-open or auto-close the Claude sheet when switching projects based on injection configuration — Completed: 2026-06-20
  ...
  - File: `toDoList_main/src/main.js`, `toDoList_main/src/projectRow.js`, `toDoList_main/src/listLogic.js`
  - Completed: YYYY-MM-DD (PR #<number>)
...
- [x] **[HIGH]** Add boot watchdog to recover from post-update white screens — Completed: 2026-06-20
  ...
  - Behavior:
    1. Boot signal: the app sets `window.__appBooted = true` once interactive chrome is in the DOM — in both terminal boot branches in `index.js` (after the signed-in shell renders, and after `showAuthModal()`), independent of Supabase hydration so a slow or failed data pull never trips the watchdog. On a successful boot it also clears the recovery counter (`sessionStorage.removeItem('todoapp_bootRecoveryAttempt')`).
    2. Detection: an inline script in `template.html` arms a one-shot ~10s timer (tunable; long enough never to false-fire on a slow cold boot, since the flag is set at shell render rather than after data load). If `window.__appBooted` is truthy or `#outerContainer` exists when it fires, do nothing. Otherwise treat the boot as failed and escalate by attempt count read from `sessionStorage` under `todoapp_bootRecoveryAttempt` (which persists across `location.reload()` within the tab). On any load where that counter is already > 0, the overlay is shown immediately so the heal sequence doesn't flash white between reloads; a successful boot clears the counter and removes the overlay.
    3. Attempt 0 (silent): set the counter to 1, show the branded "UPDATING…" overlay, call `registration.update()` if a SW registration is available, then `location.reload()`.
    4. Attempt 1 (silent, deeper bust): set the counter to 2, delete all Cache Storage entries (`caches.keys()` → `caches.delete`), unregister all service workers (`getRegistrations()` → `unregister()`), then `location.reload()` — now uncontrolled, so it fetches fresh from the network (the true hard-refresh equivalent).
    5. Attempt ≥ 2 (prompt): stop auto-reloading and render the visible recovery UI — message + "Reload" button (which performs the same deep cache+SW bust, then reloads) + a muted fallback line. No further automatic reload, so the screen can't thrash.
  - Recovery UI (option B): a full-screen overlay at a very high z-index with the Void palette hardcoded inline (bg `#0e0f14`, surface `#191a22`, accent `#8b7bff`, text `#e6e7ee`, muted `#8a8d9c`, border `#23242e`) because `style.css` isn't loaded on a failed boot; the mono caption uses a generic monospace stack (SpaceMono won't be available) and the message uses `system-ui` to match the app body font. Stage 1 is a centered accent spinner above an uppercase letter-spaced "UPDATING…". Stage 2 is a small inline-SVG ghost (accent stroke), the line "This update needs a refresh to finish.", a solid `#8b7bff` "Reload" button (white text, ~10px radius, ≥44px tap height), and a muted sub-line "Still stuck? Fully close and reopen the app."
  - Implementation notes:
    - Inline in `template.html`, not a module, so a bundle that fails to parse can't take the watchdog down with it. Wrap everything in try/catch and feature-detect `caches`, `navigator.serviceWorker`, and `sessionStorage` — the button's `location.reload()` still works without them.
    - No new `setInterval` is added to `index.js`; the watchdog uses a `setTimeout` and lives in `template.html`, so the `tests/serviceWorkerUpdate.test.js` first-`setInterval` invariant (≥15 min) is unaffected.
    - Independent of the existing update flow (`modals.js` dot, `applyPendingUpdate`, the `controllerchange → reload` listener) — that healthy path is untouched. The watchdog reloads only on a detected dead boot and are gated by the `sessionStorage` counter, so they don't compound with the existing one-shot `reloading` guard.
    - No new dependencies (vanilla inline script). No change to `sw.js`, the precache strategy, or any build config.
    - Not covered by Vitest (jsdom runs source modules, not the built HTML, and doesn't exercise Cache Storage or SW unregistration) — validate manually per the README PWA-update matrix (installed PWA on desktop Chrome, iOS Safari home-screen, Android Chrome): ship a version bump, force a failed boot, and confirm Stage 1 self-heals on the first reload and that after the cap Stage 2 shows a working Reload button. Optionally add a unit assertion that `index.js` sets `window.__appBooted` in both boot branches to lock the contract.
  - Out of scope: the existing in-app "update available" cue and its prominence (the healthy-app path); content-hashing the bundles or any change to the precache/cache strategy in `sw.js`/`webpack.config.js`; any redesign of the auth modal or main shell.
  - File: `toDoList_main/src/template.html`, `toDoList_main/src/index.js`
  - Completed: YYYY-MM-DD (PR #<number>)
...
- [x] **[MEDIUM]** Add inline project creation input at top of `projectPickerDropdown` on desktop — Completed: 2026-06-20
...
- [x] **[MEDIUM]** Restyle the desktop project-picker inline-create row as a labeled "Create" card — Completed: 2026-06-20
  - Type: feature
...
- [x] **[LOW]** Restyle the chat composer buttons to match the approved dual-send design — Completed: 2026-06-21
  ...
  - Implementation notes:
    - Buttons are built in `buildChatView()` in `claudeSheet.js`: replace the Deep button's current Claude-logo glyph with 🧠 (removing any inline logo SVG), and add the small "fast"/"deep" label elements beneath the two send buttons.
    - Shape, fill, uniform sizing, the paired send grouping, and label styling live in `style.css` — reuse the existing composer-button structure (`.claudeComposer`, `#claudeComposerSend`, `.micButton`, `.claudeAttach`) so the single-row alignment holds and nothing shifts vertically.
    - Keep the `.claudeAttach` positioning wrapper intact — the file-picker dropdown is absolutely positioned against it, so the borderless look is a styling change to that wrapper, not removal of the element.
    - Authorized: update any composer/send-button assertions the dual-send change added in `tests/claudeSheet.test.js` to match the restyle. Test surface is structural only (the Deep button still renders with its id/class); the visual treatment is CSS — don't add pixel or visual assertions.
...
- [x] **[LOW]** Remove mobileProjOpen and mobileProjDone count badges from the mobile navbar
  - Type: feature
...
- [x] **[MEDIUM]** Open to the first project on app launch instead of the last-selected one — Completed: 2026-06-22
  - Type: feature
...
- [x] **[MEDIUM]** Make the Version row in the mobile settings menu tap to open the changelog — Completed: 2026-06-21
  - Type: feature
...
- [x] **[MEDIUM]** Add a Conceive tab for incubating project ideas through staged lifecycle sections — Completed: 2026-06-21
  ...
  - Behavior:
    1. Concept index: render saved concepts newest-first (or by stored `pos`); "+ New concept" creates an untitled concept (seeded with the SDLC stage list and `lifecycle: 'SDLC'`) and opens its editor; an empty state shows when there are none. Each concept row supports rename and delete; delete shows a confirmation step naming that the concept and all its stage content will be lost (per the destructive-action convention).
    2. Concept editor: render the concept's `stages` in stored order as collapsible section rows, each showing its label and a status dot — filled when that stage's `body` is non-empty, hollow when empty. Tapping a header toggles it; the body is a multi-line text area that autosaves through the data layer on change (debounced, or on blur). Show the `lifecycle` label as a small chip beside the concept title (informational for now). A back affordance returns to the index.
    3. Navigation: the index↔editor swap is internal to the Conceive view (a mode toggle inside the module), not a new top-level view. Switching to another tab and back may reset to the index — preserving the exact open concept/mode across tab switches isn't required for this entry.
  - Implementation notes:
    - Add a fourth view token 'conceive' alongside 'projects' | 'inbox' | 'calendar': extend `applyActiveView` in `main.js` (set `data-view="conceive"` on `#mainBar` and `<body>`, sync the new pill + mobile tab active state), add a `viewPillConceive` to `viewSwitcher` and a matching `mobileTabBar` tab, append the new view container to `main2`, and add 'conceive' to the known-token allowlist in `prefs.js` (`ACTIVE_VIEW_KEY`) so it restores across reloads and a stale pref still falls back to 'projects'.
    - Put the data model in a new `conceptsLogic.js` (mirroring how `listLogic.js` owns the project model + its localStorage cache): export create/rename/delete/get/reorder plus a stage mutator that targets a stage by id — `setStageBody(conceptId, stageId, text)` — since stages are a list, not fixed keys. `create` seeds the ordered SDLC stage list and `lifecycle: 'SDLC'`. Every mutation writes the `todoapp_concepts` cache through one save helper; do not write localStorage from the view module. Concept shape: `{ id, title, lifecycle, stages: [{ id, label, body }], createdAt, updatedAt, pos }`, ids via `crypto.randomUUID()`, ISO timestamps — the ordered `stages` array serializes directly to a future Supabase `concepts.stages` jsonb column.
    - Render + wiring go in a new `conceiveView.js` (mirroring `inboxView.js` / `calendarView.js`); styling goes in `style.css` only — no inline style writes from JS. Keep stage text inputs at `font-size: 16px+` to avoid iOS Safari auto-zoom. No new dependencies — build the editor with native textareas/contenteditable, not a rich-text library.
    - `main.js` is over 25k tokens — locate the nav-construction block (`viewSwitcher`, `mobileTabBar`), `applyActiveView`, and the `main2.appendChild(...)` view-append sites with grep + offset/limit, never a full read. If any module-level listener is added, guard it against the double-evaluation the four webpack entry bundles cause (the existing `window.__hydrateListenerRegistered` one-shot pattern).
    - Tests: add a `conceptsLogic.test.js` covering create seeding the five SDLC stages in order, `setStageBody` targeting by id, the filled/empty derivation from `body`, and round-trips through localStorage. If introducing the new modules trips any existing test that text-reads source via `readFileSync`, repoint it.
  - Out of scope: the lifecycle/methodology preset picker and per-concept add/rename/remove/reorder of stages (follow-up — Run 1 seeds and renders the SDLC list and edits bodies only); Supabase persistence + cross-device sync for concepts (follow-up); the "Graduate to a project" action and seeding todos from the Build-plan stage (follow-up); per-stage "Ask Claude" assist (follow-up); removing the Inbox and Calendar views (separate follow-up).
  - File: `toDoList_main/src/conceptsLogic.js`, `toDoList_main/src/conceiveView.js`, `toDoList_main/src/main.js`, `toDoList_main/src/prefs.js`, `toDoList_main/src/style.css`, `toDoList_main/tests/conceptsLogic.test.js`
  - Completed: YYYY-MM-DD (PR #<number>)
...
- [ ] **[MEDIUM]** Remove the Inbox and Calendar views, leaving Projects and Conceive
  ...
  - What to remove:
    1. Modules: delete `toDoList_main/src/inboxView.js` and `toDoList_main/src/calendarView.js` in full.
    2. `main.js` imports + wiring: the `renderInbox` import and the `calendarView.js` import surface (`renderCalendarView`, `resetCalendarStateToToday`, `shiftCalendarMonth`, `formatCalendarKeyForDate`, `getCalendarSelectedKey`, `initCalendarView`); the `initCalendarView({ applyActiveView })` call at module load; the `'inbox'` and `'calendar'` branches in `applyActiveView` (the switch should resolve only `'projects'` and `'conceive'`); `viewPillInbox` + `viewPillCalendar` in `viewSwitcher` and their mobile-tab-bar counterparts; the `main2.appendChild(inboxView)` / `main2.appendChild(calendarView)` lines and the two container elements built in `component()`; the calendar month prev/next buttons and their click handlers; the `calendarNavArrowKey` function and its `keydown` bindings; the Calendar arrow-key re-focus block that reads `getCalendarSelectedKey()`; and the `'inbox'` / `'calendar'` branches in `firstFocusableInActiveMainView`. Update any remaining nav-order arrays that enumerate the pills to drop `viewPillInbox` / `viewPillCalendar` and keep the survivors (`viewPillProjects`, `viewPillConceive`).
    3. `prefs.js`: drop `'inbox'` and `'calendar'` from the `ACTIVE_VIEW_KEY` known-token allowlist so only `'projects'` and `'conceive'` validate, and a stored `'inbox'` / `'calendar'` value (from existing installs) falls back to `'projects'`. Update the now-stale "three top-level views" comment.
    4. `style.css`: remove the inbox selectors (`.inboxRow*`, `.inboxRowCheck`, `.inboxRowBody`, `.inboxRowTitle`, `.inboxRowMeta`, `.inboxRowProject`, `.inboxRowChev`, `.inboxEmptyState`, `#inboxView`, `#inboxSections`, `[data-view="inbox"]`) and the calendar selectors (`#calendarView`, `#calendarGrid`, calendar cells, `.todayRow*`, the prev/next button styles, `[data-view="calendar"]`, day-panel styles). Before deleting any class, grep it in source to confirm it isn't shared with the Projects view — in particular preserve `.todoRowCard` if it's still used by project rows; only the `.todayRow`-specific rules go.
    5. `listLogic.js`: if a grep confirms `getIdeaTodosAcrossProjects` has no callers left once `inboxView.js` is gone, remove it; if anything else references it, leave it. Do NOT remove the recurrence helpers (`advanceRecurringTodo`) or the calendar-key formatter — they're used by the Projects view — unless grep proves them fully orphaned.
    6. Tests: delete the inbox tests (`inboxIdeasView.test.js`, `inboxRowTap.test.js`, `inboxViewRename.test.js`) and the calendar tests (`calendarKeyboardNav.test.js` plus the calendar CSS-only tests under `tests/`). Delete `inboxIdeasQuery.test.js` only if `getIdeaTodosAcrossProjects` is removed in step 5. Keep the Projects-view IDEAS-filter test in `taskFilter.test.js` — it pins the non-regression below.
  - Acceptance criteria:
    - `main.js` no longer imports from or references `inboxView.js` / `calendarView.js`, and neither module file remains; no dangling references to any removed symbol anywhere in source or tests.
    - The tab bar offers exactly Projects and Conceive on both desktop sub-band and mobile tab bar; restoring the app with a stale `'inbox'` or `'calendar'` pref lands on Projects.
    - Projects view is visually and behaviorally unchanged: status-chip popover, ALL/Active/Ideas filter pills and their counts, due-date pills, and the recurring-task checkbox all still work; setting a task to `idea` and filtering to Ideas still shows it.
    - Full Vitest suite (`npm run test:run`) green.
  - Implementation notes: `main.js` is over 25k tokens — locate the blocks with grep + offset/limit (the `applyActiveView` switch, the `viewSwitcher` / mobile-tab construction, the `main2.appendChild` view-append sites, `calendarNavArrowKey`, the prev/next-month handlers, the arrow-key re-focus block, and `firstFocusableInActiveMainView`), never a full read. Tear down additively: strip the wiring and confirm the suite is green, then delete the now-unreferenced module files and CSS. This also retires the open "first Enter on Calendar jumps focus to today" bug, since the Calendar view no longer exists.
  - Out of scope: any change to the Projects view, the status/`idea` data model, due-date or recurrence logic, the Conceive view, or the TODO.md viewer band; relocating the cross-project ideas roll-up or the month grid elsewhere (both are intentionally dropped).
  - File: `toDoList_main/src/main.js`, `toDoList_main/src/prefs.js`, `toDoList_main/src/style.css`, `toDoList_main/src/inboxView.js`, `toDoList_main/src/calendarView.js`, `toDoList_main/src/listLogic.js`, `toDoList_main/tests/inboxIdeasView.test.js`, `toDoList_main/tests/inboxRowTap.test.js`, `toDoList_main/tests/inboxViewRename.test.js`, `toDoList_main/tests/inboxIdeasQuery.test.js`, `toDoList_main/tests/calendarKeyboardNav.test.js`
  - Completed: YYYY-MM-DD (PR #<number>)
  <!-- id: a56a39ab-29bb-4fc3-aafc-0ea2cd8554da -->
