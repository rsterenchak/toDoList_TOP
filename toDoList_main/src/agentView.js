...
// persistent working watch they seed now lives in the store too, so it is no longer
// injected here). Same hoisting + acyclic rationale as configureAssignmentCoverage
// above: the
...
// its in-view unavailable message. The persistent working watch the gate polls on
// a project switch now lives in the store too, so that recompute is a local call
// there — the board no longer registers it.
