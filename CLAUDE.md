...
- **(c) Iterate seed** — the iterate seed is sent as `body.entry_id`, on **every turn** of an active iterate session, not just the first. The Worker resolves that entry's marker to a merged PR and assembles a seed: the PR diff plus sliced post-merge source (it caches and re-serves that seed cheaply on follow-up turns). Typical cost ~12–20k input tokens. Invoked by tapping a **SHIPPED run record** in the Runs tab, which opens an iterate chat seeded from that merged change. The active entry id is held in `activeIterateEntry` and persisted per repo under `todoapp_claudeIterateEntry` (in lockstep with the per-repo chat history), so follow-ups, INSPECT measurement turns, and a reload mid-iterate all keep the diff; the seed turn establishes it on success and clears it on a 404, "+ New Chat" and a workspace swap clear/swap it, and the No-change follow-up path sends no `entry_id`. See `requestAssistantReply` / `chatWithWorker` in `claudeSheet.js` / `inject.js`.
...
- `toDoList_main/src/ghostTalk.js` — the ghost's tap-to-talk surface, shared by both ghost mounts: the speech bubble and ask input docked to the frozen ghost, wired to the Worker's `ghost` route through `postToWorker`. Each mount names its surface (`desktop` / `mobile`) when it opens.
- `toDoList_main/src/mobileGhost.js` — the mobile ghost perch: swipe up on the bottom tab bar to summon it into the bottom-left corner, tap it to open the talk surface, swipe down to dismiss. Visibility persists under `todoapp_mobileGhostVisible`.
