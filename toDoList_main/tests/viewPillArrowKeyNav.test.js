...
// Pins the contract for the ArrowDown / ArrowUp boundary between the two
// view-switcher pills (PROJECTS / CONCEIVE) and the visible main pane
// below them. Mirrors the sidebarToggle ↔ first-project-row pattern so
// keyboard users can flow vertically out of the header chrome into the
// rendered items without grabbing the mouse:
//   • ArrowDown from a pill drops focus into the visible main pane's
//     first focusable element (placeholder #toDoInput in PROJECTS).
...
describe('view-pill arrow-key navigation into and out of the main pane', () => {
    ...
    function extractViewPillKeydown() {
        // The handler is wired on both pills via a shared function;
    ...
    it('wires a keydown listener on viewPillProjects that runs the pill-down handler', () => {
        ...
        // from the Projects pill.
        expect(main).toMatch(/viewPillProjects\.addEventListener\(\s*['"]keydown['"]/);
    });

    it('wires a keydown listener on viewPillConceive that runs the pill-down handler', () => {
        // Mirror of the Projects wiring — keyboard users arriving at the
        // Conceive pill via ArrowLeft/ArrowRight need the same vertical
        // drop-in affordance.
    ...
    it('the pill drop-in handler picks its target from the active view', () => {
        ...
        // Resolving the target via the active-view-aware helper means the
        // pill ArrowDown lands on whichever surface is currently visible —
        // the placeholder input in PROJECTS.
