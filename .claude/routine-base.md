...
4. Your local <BASE_BRANCH> is up to date with origin. If not, pull before proceeding. (This is the only concurrency guard — auto-merge means there should be no lingering Claude PRs, so the previous open-PR check is no longer needed.)
...
5. Open a PR against <BASE_BRANCH>:
   ...
   - Body, in this order:
     ...
     • **Notes** — pre-existing failures, follow-up items added to TODO.md, assumptions made, tasks skipped as vague or as missing/malformed Type, any tests updated (with one-line justification each).
   - Ready for review, not draft.

6. Auto-merge the PR using a **merge commit** strategy (preserves the per-task branch history as a merge node on main). Use `gh pr merge <number> --merge --delete-branch`. This both completes the merge and deletes the remote branch in one step. If the merge fails (conflict, branch protection requires reviews, status checks not yet reported), stop and report the exact error — do not attempt creative workarounds. Do not delete the branch manually if the merge itself failed; leave the branch and PR in place for human inspection.
...
- Never push or open a PR without running the local test suite first and seeing it pass. There is no WIP escape hatch — a red suite means the routine aborts per <test_verification> step 5, leaving main and the local workspace untouched.
...
- If any step fails irrecoverably (push rejected, PR API error, merge conflict with base, auto-merge blocked), stop and report the failure with the exact error. Do not try creative workarounds.
</hard_constraints>
