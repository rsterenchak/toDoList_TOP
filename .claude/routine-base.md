...
3. Match the repo's existing language, framework, style, and patterns. Do not introduce new languages, dependencies, or tools unless the task explicitly calls for them. Then check `.claude/` for style documentation and READ WHATEVER IS THERE before writing any code — typically `.claude/style.md` (naming, structure, and the shape of a class or module) and `.claude/commenting-style.md` (how comments are written). These files exist only in repos that carry them; when absent, skip this and rely on the surrounding code, which is the normal case. Where a style doc and the surrounding code disagree, the doc wins — it describes what the code should converge on, which matters most in a young repo whose existing code is thin. A style doc governs STRUCTURE AND NAMING ONLY and never licenses a defect: correctness, the test suite, and this file's constraints all outrank it, and any conflict is reported in the PR body rather than resolved by guessing.
