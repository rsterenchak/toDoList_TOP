...
//     (hamburger, PROJECTS / CONCEIVE pills, pomodoro, music, settings).
...
describe('header / footer arrow-key navigation', () => {
    ...
    it('nav handler walks all six header controls in on-screen order', () => {
        ...
        // view-switcher pills (PROJECTS, CONCEIVE — that's the sequence
        // #viewSwitcher appendChilds them in), pomodoro, music, settings.
        // A different order would make ArrowRight land on the wrong neighbor
        // relative to where focus appears on screen.
        const seq = body.match(/sidebarToggle[\s\S]*?viewPillProjects[\s\S]*?viewPillConceive[\s\S]*?pomodoroToggle[\s\S]*?musicToggle[\s\S]*?settingsToggle/);
