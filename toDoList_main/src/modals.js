...
import { makeInjectButton, refreshInjectButton, writeAssignmentToWorker, readAssignmentFromWorker, findTargetById, TODO_RUN_STATUS_EVENT } from './inject.js';
...
// owns the async load-and-cache; here we only read what it already cached).
...
// ── FILE:-LINE INSERTION ──
// A repo-relative source path is present on a `File:` line if it appears as one
...
// path the inject button uses) so the File:-path picker can read that repo's
// cached manifest. Returns null when the project has no routing target, in
// which case the picker action stays hidden. Mirrors resolveProjectRepo in
// seedTasksModal.js, replicated here to avoid a modals ↔ seedTasksModal import
// cycle.
function resolveModalRepo(projectName) {
...
export function showDescEditorModal(item, options) {
    ...
    // your head. Reads the manifest SYNCHRONOUSLY from the cache: if the active
    // project has no linked repo, or that repo's manifest was never loaded (CI
    // hasn't published it, or Structure was never opened this session), the chip
    // stays hidden entirely rather than opening an empty picker or blocking the
    // modal on a fetch. The chip sits above the textarea; its searchable panel
    // drops in below it, capped so a large manifest can't mount hundreds of rows.
    const targetRepo = resolveModalRepo(opts.projectName || '');
    const cachedManifest = getCachedManifest(targetRepo);
    const manifestFiles = (cachedManifest && cachedManifest.ok && Array.isArray(cachedManifest.files))
        ? cachedManifest.files
        : [];

    const targetPickBtn = document.createElement('button');
    targetPickBtn.id = 'descEditorModalTargetPick';
    targetPickBtn.type = 'button';
    targetPickBtn.textContent = '⌖';
    targetPickBtn.setAttribute('aria-label', 'Pick a file path from the project');
    targetPickBtn.setAttribute('aria-expanded', 'false');
    // Hidden entirely when there is no manifest to browse.
    if (!manifestFiles.length) targetPickBtn.style.display = 'none';
    body.insertBefore(targetPickBtn, textarea);

    const targetPanel = document.createElement('div');
    targetPanel.id = 'descEditorModalTargetPanel';
    targetPanel.hidden = true;

    const targetSearch = document.createElement('input');
    targetSearch.id = 'descEditorModalTargetSearch';
    targetSearch.type = 'text';
    targetSearch.setAttribute('aria-label', 'Filter files');
    targetSearch.placeholder = 'Filter files…';
    targetSearch.autocomplete = 'off';
    targetSearch.spellcheck = false;

    const targetList = document.createElement('div');
    targetList.id = 'descEditorModalTargetList';
    targetList.setAttribute('role', 'listbox');

    targetPanel.appendChild(targetSearch);
    targetPanel.appendChild(targetList);
    body.appendChild(targetPanel);

    // Cap the rendered rows so a several-hundred-file manifest never mounts in
    // full on a phone; when the filter still matches more than the cap, show a
    // "keep typing to narrow" hint instead of the overflow.
    const TARGET_PICK_CAP = 60;

    function applyFilePick(path) {
        textarea.value = insertFilePathIntoEntry(textarea.value, path);
        // Route the change through the textarea's own input listener (re-syncs
        // item.desc, refreshes the inject button, recalculates auto-grow), then
        // persist so the description survives a reload.
    ...
    function renderTargetList() {
        ...
        if (!matches.length) {
            ...
            empty.className = 'descEditorModalTargetEmpty';
        ...
        matches.slice(0, TARGET_PICK_CAP).forEach(function (path) {
            ...
            row.type = 'button';
            row.className = 'descEditorModalTargetRow';
        ...
        if (matches.length > TARGET_PICK_CAP) {
            ...
            more.className = 'descEditorModalTargetEmpty';
            more.textContent = 'Keep typing to narrow ' + matches.length + ' matches';
            targetList.appendChild(more);
        }
    }

    function openTargetPanel() {
        targetPanel.hidden = false;
        targetPickBtn.setAttribute('aria-expanded', 'true');
        renderTargetList();
        try { targetSearch.focus(); } catch (e) { /* defensive */ }
    }
    ...
    targetPickBtn.addEventListener('click', function () {
        if (targetPanel.hidden) openTargetPanel();
