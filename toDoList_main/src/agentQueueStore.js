...
// trackers reach the board's pill repaint and the Worker probes ONLY through
// callbacks registered via configureRunTrackers — the store must not import a view
// or inject.js (the same TDZ / acyclic rule the reconciler follows). The persistent
// working watch they seed now lives in this module too (below), so that seed is a
// direct local call rather than a registered callback.
...
export function startSweepTracking(alreadyConfirmed) {
    ...
    // lives in this module (below), so this is a direct local call.
...
// selected project. It lives here beside the dispatch reconciler and the run
// trackers it mirrors: like them it must keep working with no board mounted, so
// it cannot live in agentView.js (torn down on tab-exit). The board subscription
// is torn down by unsubscribeAgentView() on tab-exit (a backgrounded board holds
// no open socket), which also clears the board's rows and forces its
// `_sweepActive` false via stopSweepTracking — so the header pill's working signal
// goes dark the instant you leave the Agent tab, exactly the context the nav
// "working" dot exists to serve. This watch is therefore started once at app init
// and NEVER torn down. It carries ONLY the working signal: its own minimal
// agent_queue realtime subscription (separate from the board channel, to catch
...
// pattern. Its two out-of-store capabilities — the routed dispatch target and the
// Worker active-runs probe — are reached through the SAME configureRunTrackers DI
// channel the run trackers use (_trackerDeps.resolveDispatchTarget /
// .fetchActiveRuns), since the store must not import a view or inject.js.
...
// so it holds off-tab where the board's `_rows` / `_sweepActive` are cleared. Both
// halves are scoped to the selected project: the ship half reads its
...
// (see below). The sweep component then folds the raw probe with any local seed via
...
// simply resolves false). The routed target and the active-runs probe are read
// through the run-tracker DI channel, so before the board registers them the sweep
// half is skipped (target resolves null).
...
//
// The gate reaches inject configuration — which lives outside the store — through
// the store's established DI channel rather than a static import, since the store
// must not import a view or inject.js (the same acyclic rule the reconciler and run
// trackers already follow); it recomputes the nav dot through a now-local call:
...
//   • pollAgentWorkingWatch() — the persistent working watch now lives in this
//     module (above), so the gate recomputes the nav dot with a direct local call
//     rather than through a registered callback.
...
export function syncAgentAvailabilityForProject(projectName) {
    ...
    // documented project-switch hook, so recompute here. The watch is local to this
    // module now, so this is a direct call.
    pollAgentWorkingWatch();
    return hasRepo;
}
