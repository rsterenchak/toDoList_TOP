...
describe('music — main.js wiring', () => {
    ...
    it('creates a #musicToggle button between pomodoro and settings in the nav', () => {
        ...
        // before settingsToggle. The focus-mode and API-spend toggles sit between
        // music and settings in the right cluster. A regex catches this ordering.
        expect(main).toMatch(/pomodoroToggle\s*\)\s*;\s*nav\.appendChild\(\s*musicToggle\s*\)\s*;\s*nav\.appendChild\(\s*focusModeToggle\s*\)\s*;\s*nav\.appendChild\(\s*spendToggle\s*\)\s*;\s*nav\.appendChild\(\s*settingsToggle/);
    });
