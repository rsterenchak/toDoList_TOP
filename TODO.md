...
- [x] **[HIGH]** Skip the row's focus-the-input handler when the click target is the status label — Completed: 2026-06-07
  - Type: bug
  - Description: Clicking a `.todoStatusLabel` mounts the status popover and then tears it down within the same tick, so the popover never visibly opens. Root cause is an event-chain interaction confirmed by a console trace (`hideStatusPopover` called from a `scroll` event whose source frame is `toDoRow.js:464`): the row's main click handler in `toDoRow.js` (the `toDoChild.addEventListener('click', ...)` body) runs in bubble phase BEFORE the delegated handler on `#mainList`, and calls `toDoInput.focus()` even when the click target is the status label. The browser's focus-into-view scroll then fires AFTER the delegated handler mounts the popover, and the scroll listener `showStatusPopover` just registered (`window.addEventListener('scroll', hideStatusPopover, true)`) treats that involuntary scroll like a user scroll and removes the popover. Rows whose focused input happened to already be perfectly in view skip the involuntary scroll, which is why the bug looked status-specific — `in_progress` rows happened to sit at positions where `focus()` didn't shift the viewport, so they "worked." The MutationObserver diagnostic showed every status flashes the popover and removes it. Fix by treating the status label as a sub-control in the row's click handler, exactly like `copyBtn` already does: add an `event` parameter and early-return when the click target is inside a `.todoStatusLabel`. The label's own popover wiring (delegated handler on `#mainList`) still runs because it's in the bubble path past this return; the only thing being skipped is the input-focus side effect that triggers the scroll.
  - Behavior:
    1. Clicking a `.todoStatusLabel` on any status (`active` / `in_progress` / `idea`) mounts the popover and KEEPS IT MOUNTED until a real dismissal vocabulary event fires (option select, outside click, Escape, true user scroll, resize).
    2. Clicking the label does NOT focus `#toDoInput` and does NOT add `todo-active` to the row — sub-control clicks are conceptually independent of the row's "activate to edit" path, matching the `copyBtn` precedent.
    3. Clicking anywhere else in the row (title display, blank padding area) still focuses the input and activates the row exactly as before — only the label is excluded.
    4. The fix is location-agnostic: the row's input does NOT need to be in view for the popover to persist. A row scrolled to mid-viewport, top, or bottom all behave identically after the click.
    5. The popover's existing scroll dismissal stays — a genuine user scroll (wheel, trackpad, keyboard arrow on `#mainList`) still dismisses, as intended.
  - Test-first regression set (behavioral, against real DOM events — source-pattern alone is insufficient for this class of bug per the rename-saga lesson):
    1. Click a `.todoStatusLabel` on an `active` row → after the click, `document.getElementById('todoStatusPopover')` is non-null AND remains non-null after a `Promise.resolve()` flush AND after a `requestAnimationFrame` tick (covers both microtask- and frame-deferred scroll dispatch).
    2. Same as above for `in_progress` and `idea` rows — pin all three statuses so a future regression that re-introduces a status-specific bypass fails here.
    3. After a label click, `document.activeElement` is NOT the row's `#toDoInput`, and the row does NOT carry `todo-active`. The label tap is independent of the row's edit path.
    4. Clicking the row's title (`.toDoTitleDisplay` or the input area) still focuses `#toDoInput` and still adds `todo-active` — regression guard against the early-return matching too broadly.
    5. A synthetic `window.dispatchEvent(new Event('scroll'))` AFTER the popover is mounted from a label click STILL dismisses it — proves the popover's intentional scroll dismissal vocabulary is intact and we haven't blunted it.
    6. Source-pattern supplement (not a substitute): the row click handler's body contains `event.target.closest('.todoStatusLabel')` returning early before the `toDoInput.focus()` call site.
  - Implementation notes: The change is local to the `toDoChild.addEventListener('click', function() {...})` body in `toDoRow.js` (the function around line 464 per the trace — grep for `// one-click editing` in the file to land on it cleanly). Two edits: (a) add `event` as the parameter, (b) add `if (event && event.target && event.target.closest && event.target.closest('.todoStatusLabel')) return;` as the first statement inside. Do NOT touch `wireStatusLabelDelegation`, `showStatusPopover`, or `hideStatusPopover` — they're correct as-is; the scroll dismissal is intentional for user-initiated scrolls and removing it would create a different bug (popovers stranded off-anchor). Do NOT switch the input focus call to `focus({ preventScroll: true })` — that would mask the symptom but the deeper bug (sub-control clicks running the row's activation path) would still affect any future sub-control. The previous "aria-expanded check" fix in `wireStatusLabelDelegation` for the cross-label race stays and is independently correct.
  - Out of scope: any change to the popover module; any change to other sub-controls (checkbox, due pill, desc toggle — they each handle their own propagation already); generalising the sub-control exclusion to a broader `.todoSubControl` class (worth doing later, not in this fix); the focus-into-view behavior of inputs in general.
  - File: `toDoList_main/src/toDoRow.js`, `toDoList_main/tests/todoStatus.test.js` (or the existing `toDoRow` test file if one exists — grep first)
  - Completed: YYYY-MM-DD (PR #<number>)
...
- [x] **[LOW]** Match the filter/sort band background to the view-tabs sub-band — Completed: 2026-06-07
  ...
  - Behavior:
    1. At ≥1024px (where the view-tabs sub-band is visible), the filter+sort band and the view-tabs sub-band paint identically — no visible seam, no `background-color` mismatch, no border line between them.
    2. The top project-switcher row's greyer chrome (the row holding the project pill + open/done stats) is UNCHANGED — this entry intentionally preserves it so the top header reads as a distinct band above the now-merged view-tabs + filter/sort area.
    3. The filter pill's own purple selected-state background is unchanged. The sort dropdown's own border/background is unchanged. Only the row's *outer wrapper* changes — children keep their existing chrome.
    4. At mobile widths (<1024px) the view-tabs sub-band is `display:none`, so this rule is a no-op there — but verify the mobile filter band still paints whatever it paints today (no regression for the mobile-only chrome).
  - Implementation notes: Grep `style.css` for `#taskFilterBar` and the sort-dropdown wrapper (likely `#taskSortControl` or similar — the sort-dropdown entry's selectors landed recently) to find the shared parent at the desktop breakpoint. The fix is almost certainly removing one `background:` declaration or replacing it with `background: transparent;` to inherit the page bg. Don't introduce a new CSS variable for this — `#desktopViewSubBand` doesn't set a background and neither should this band; matching the absence is the goal. Behavioral test (extends the existing taskFilter / sort test files — grep first): assert the filter+sort band's `getComputedStyle(...).backgroundColor` equals `#desktopViewSubBand`'s at viewport ≥1024px. Also assert the top project-switcher row's `backgroundColor` is UNCHANGED (capture its current computed value and pin it) so a future "match everything to the page bg" refactor doesn't silently strip the top-row chrome this entry is preserving. Source-pattern alone is brittle here (a child's bg could mask the wrapper's transparency); computed-style is the right level.
  - Out of scope: any change to the view-tabs sub-band; the top project-switcher row's greyer background (intentionally preserved); the filter pill's own background; the sort dropdown's own chrome; the page background variable itself; mobile band chrome.
...
- [x] **[HIGH]** Revert #taskFilterBar background to transparent (previous entry shipped the inverse) — Completed: 2026-06-07
  ...
  - Behavior:
    1. At ≥1024px: `getComputedStyle('#taskFilterBar').backgroundColor` is either `rgba(0, 0, 0, 0)` (transparent — preferred) or matches `getComputedStyle('#mainBar').backgroundColor` exactly. Either form makes the band visually identical to the view-tabs sub-band above it.
    2. The top project-switcher row (the row holding the project pill + open/done stats) keeps its current greyer chrome (`rgb(20, 21, 27)`-ish, from `#outerContainer`) — pin this with a computed-style assertion so a future "strip all backgrounds" sweep can't take it down by accident.
    3. The cycle pill (purple) and sort dropdown (purple-bordered) inside the band keep their own chrome — only the band's own background changes.
    4. At <1024px the view-tabs sub-band is `display:none`; verify the band still paints whatever it does today on mobile (regression guard, not a behavior change).
  - Implementation notes: Grep `style.css` for `#taskFilterBar { ... background:` to find the rule the previous entry left in place. If multiple matches exist (e.g., a desktop-scoped block and a base block), the desktop one is the active culprit at ≥1024px — confirm by checking which selector wins `getComputedStyle` resolution. The cleanest fix is to delete the `background:` declaration entirely so the band inherits naturally; `background: transparent;` is the safer alternative if any other rule competes. Do NOT introduce a new variable or rename existing tokens — `#mainBar` already owns the page bg this band should match. Behavioral test: assert `getComputedStyle(taskFilterBar).backgroundColor` equals `getComputedStyle(mainBar).backgroundColor` (or is `rgba(0, 0, 0, 0)`) at viewport ≥1024px, AND that `#outerContainer`'s computed background is preserved at its current value so the top-row chrome stays intact. Source-pattern alone is brittle for this — the previous entry passed source-pattern tests while shipping the inverse behavior.
  - Out of scope: any change to `#outerContainer`, `#mainBar`, the top project-switcher row's chrome, the view-tabs sub-band, the cycle pill, the sort dropdown, the mobile band; the page bg variable itself.
  - File: `toDoList_main/src/style.css`, `toDoList_main/tests/taskFilter.test.js` (or the closest existing band-layout test file — grep first)
  - Completed: YYYY-MM-DD (PR #<number>)
  <!-- id: 46f60e5e-b79c-48f2-a438-d6a968b8feed -->

- [ ] **[MEDIUM]** Pull #desktopViewSubBand down onto the #mainBar page bg
  - Type: bug
  - Description: The previous "match the filter+sort band to the view-tabs sub-band" entry plus its revert left both `#taskFilterBar` and `#desktopViewSubBand` set to `background: transparent` — which is the correct end-state for each — but the two elements live in DIFFERENT parents, so their `transparent` resolves to different colors. Walking ancestors confirms it: `#taskFilterBar`'s parent is `#mainBar` at `rgb(14, 15, 20)` (the page bg), but `#desktopViewSubBand`'s parent is `#outerContainer` at `rgb(20, 21, 27)` (the greyer chrome). Result: the filter band paints page bg as intended, but the view tabs paint the greyer color, so the seam is still visible — just in the opposite direction from before. Fix by relocating `#desktopViewSubBand` from its current position (under `#outerContainer`) to under `#mainBar`, so it inherits the same parent background the filter band already does. The top project-switcher row stays under `#outerContainer` and keeps its greyer chrome unchanged — only the view-tabs sub-band moves down a level in the parent chain.
  - Behavior:
    1. After the change, `#desktopViewSubBand.parentElement === document.getElementById('mainBar')`. Pin this with an explicit assertion — DOM parentage is the actual fix, not a color override.
    2. At ≥1024px: `getComputedStyle('#desktopViewSubBand').backgroundColor` equals `getComputedStyle('#taskFilterBar').backgroundColor` equals `getComputedStyle('#mainBar').backgroundColor` (`rgb(14, 15, 20)`). One band, one color, no seam.
    3. The top project-switcher row (project pill + open/done stats) stays under `#outerContainer` and its computed background is UNCHANGED at `rgb(20, 21, 27)`. Pin this — the prior round's failure mode was sweeps that ended up taking the top row's chrome down by accident.
    4. The view tabs themselves (the PROJECTS / INBOX / CALENDAR buttons) still work and still apply the same `applyActiveView()` path on click — moving the band's DOM parent does not re-wire any handlers (they're delegated or attached on the buttons themselves, not on the band).
    5. The 16px breathing margin between the top header and the sub-band that's currently driven by `margin-top:16px` on `#desktopViewSubBand` still produces the same visible gap after the move — verify visually that the spacing matches, and if the move changes the cascade source of that margin, replicate it on the new parent rule rather than re-styling the sub-band.
    6. At <1024px the sub-band is `display:none` already; verify the mobile layout is unchanged after the parent move.
  - Implementation notes: Two-part fix. (1) In whichever file builds the header DOM (grep `desktopViewSubBand` in `main.js` and related — the element is created and appended somewhere on bootstrap), change the `appendChild` / `insertBefore` site so the sub-band lands as a child of `#mainBar` instead of `#outerContainer`. (2) In `style.css`, confirm `#desktopViewSubBand { background: transparent; }` (or no `background:` declaration) is still in place — it should be after the prior revert; if not, add it back. Do NOT add a background color to either band; the goal is for both to inherit `#mainBar`'s color and the only knob that needs turning is the DOM parent. Behavioral test (extends the existing band-layout test file — grep first): assert `#desktopViewSubBand.parentElement` is `#mainBar` after `component()` boot; assert `getComputedStyle(viewSubBand).backgroundColor === getComputedStyle(taskFilterBar).backgroundColor === getComputedStyle(mainBar).backgroundColor`; assert `getComputedStyle(outerContainer).backgroundColor` is preserved at `rgb(20, 21, 27)` (top-row chrome guard). Source-pattern alone is too weak for this — the prior entry's tests asserted "the two backgrounds are equal" and passed when both were the wrong color; the new entry's tests pin the actual target color via the page-bg anchor.
  - Out of scope: any change to `#outerContainer`, `#mainBar` itself (only its child set changes), the top project-switcher row's chrome, the view tabs' button styling, the cycle pill, the sort dropdown, the mobile band; the page bg variable itself.
  - File: `toDoList_main/src/main.js`, `toDoList_main/src/style.css`, `toDoList_main/tests/taskFilter.test.js` (or the closest existing band-layout test file — grep first; if `main.js` builds the band in a function that's already source-tested, extend that file too)
  - Completed: YYYY-MM-DD (PR #<number>)
  <!-- id: 4b7dec51-d25e-4148-9f01-24f0cde864e0 -->
